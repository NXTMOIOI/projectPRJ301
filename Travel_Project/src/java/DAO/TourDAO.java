...
public class TourDAO extends DBContext {
    ...
    public int countTours(String keyword, String destination) {
        StringBuilder sql = new StringBuilder("SELECT COUNT(*) FROM Tours WHERE 1 = 1");
        List<String> params = new ArrayList<>();
        appendTourFilters(sql, params, keyword, destination);
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql.toString())) {
            ...
            throw new RuntimeException("Cannot count tours", ex);
        }

        return 0;
    }

    public List<Tour> searchTours(String keyword, String destination, int offset, int pageSize) {
        List<Tour> tours = new ArrayList<>();
        StringBuilder sql = new StringBuilder(
                "SELECT TourID, TourName, Destination, Duration, Price, MaxPeople, StartDate, EndDate, ImageURL "
                + "FROM Tours WHERE 1 = 1");
        List<String> params = new ArrayList<>();
        appendTourFilters(sql, params, keyword, destination);
        sql.append(" ORDER BY StartDate ASC, TourID DESC OFFSET ? ROWS FETCH NEXT ? ROWS ONLY");
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql.toString())) {
            ...
            throw new RuntimeException("Cannot search tours", ex);
        }
    ...
    public List<String> getDestinations() {
        ...
        String sql = "SELECT DISTINCT Destination FROM Tours WHERE Destination IS NOT NULL AND LTRIM(RTRIM(Destination)) <> '' ORDER BY Destination";
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql);
            ...
            throw new RuntimeException("Cannot get destinations", ex);
        }

        return destinations;
    }

    public boolean hasDependencies(int tourID) {
        String sql = "SELECT "
                + "(SELECT COUNT(*) FROM Bookings WHERE TourID = ?) + "
                + "(SELECT COUNT(*) FROM TourAssignments WHERE TourID = ?) + "
                + "(SELECT COUNT(*) FROM TourHotels WHERE TourID = ?) + "
                + "(SELECT COUNT(*) FROM TourVehicles WHERE TourID = ?) + "
                + "(SELECT COUNT(*) FROM Itineraries WHERE TourID = ?) AS RefCount";
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    return rs.getInt("RefCount") > 0;
                }
            }
        } catch (SQLException ex) {
            throw new RuntimeException("Cannot check tour dependencies", ex);
        }
    ...
    private void appendTourFilters(StringBuilder sql, List<String> params, String keyword, String destination) {
        if (keyword != null && !keyword.trim().isEmpty()) {
            sql.append(" AND (TourName LIKE ? OR Destination LIKE ?)");
